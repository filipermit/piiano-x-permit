...
async function verifyAndFetchDetails(req, res) {
    ...
    var checkPersonalInfo = await access.isUserAllowed(reqUid, "view-personal-info", "card");
    ...
    if (checkPersonalInfo || reqUid == uid) {
        var info = await db.query('SELECT title,description,country,badges FROM users JOIN \
		(SELECT user_email,json_agg(json_build_object(\'label\', label, \'emoji\', emoji)) as badges FROM badges GROUP BY user_email) as aa \
		ON email=user_email WHERE email=$1', [uid]);
